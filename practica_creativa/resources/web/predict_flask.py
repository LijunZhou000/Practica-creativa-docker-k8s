...
def classify_flight_delays_realtime_response(unique_id):
  ...
  consumer = KafkaConsumer('flight-delay-ml-response', bootstrap_servers=['kafka:9092'],api_version=(0,10), auto_offset_reset='latest')
  prediction = json.loads(next(consumer).value.decode('utf-8'))
